...
## OpenHands destinations

These asset systems are distinct and must not be mixed:

- Shareable product skills: `skills/<name>.md`, Markdown prompts governed by `skills/README.md`.
- Repository-private OpenHands V1 instructions: `.openhands/skills/<name>.md`.
- Repository-private OpenHands V0 compatibility instructions: `.openhands/microagents/<name>.md`.
- `.agents/skills/*/SKILL.md` is not a destination in this repository.

The Process-to-Skill converter must choose exactly one destination based on whether the result is a shareable product prompt or a repository-private instruction.
